/**
 * 18-language expert strings (PAN-INDIA) — extracted from web ExpertDashboard.jsx.
 * Expo UI-only keys live in EXPO_UI_EN; merged with per-language overrides; missing keys fall back to English.
 */

/** @type {Record<string, Record<string, string>>} */
export const EXPERT_VOICE_DICT = {
...
const EXPO_UI_EN = {
  ...
  kshatrWalletLabel: 'Kshatr wallet',
  kshatrWalletHint: 'Prepaid balance for leads & commission',
  lifetimeEarningsLabel: 'Lifetime earnings',
  lifetimeEarningsHint: 'From completed jobs (expert payout)',
  jobsSection: 'Jobs',
  activeTab: 'Active',
  completedTab: 'Completed',
  newJobActivity: 'New job activity',
  tapToDismiss: 'Tap to dismiss — list updated.',
  language: 'Language',
  ...
  emptyCompletedSub: 'Finished work will show up here.',
  serviceFallback: 'Service',
  call: 'Call',
  finishWorkTitle: 'Finish Work?',
  finishWorkMsg: 'Are you sure the job is fully completed?',
  cancel: 'Cancel',
  ...
  toastAccepted: 'Job accepted.',
  toastStarted: 'Work started.',
  toastCompleted: 'Marked completed.',
  walletDetailsTitle: 'Wallet Details',
  viewWalletDetails: 'Tap to view transactions',
  totalBalance: 'Total Balance',
  rechargeWallet: 'Recharge Wallet',
  rechargeAmountLabel: 'Enter recharge amount',
  rechargeAmountPlaceholder: 'e.g. 500',
  rechargeProceed: 'Proceed to Pay',
  paymentPending: 'Payment Gateway Integration Pending',
  paymentCancelled: 'Payment cancelled.',
  paymentFailed: 'Payment failed. Please try again.',
  paymentInitFailed: 'Unable to start payment. Please try again.',
  rechargeSuccess: 'Wallet recharge successful.',
  invalidRechargeAmount: 'Please enter a valid amount.',
  recentTransactions: 'Recent Transactions',
  noTransactions: 'No wallet transactions yet.',
  transactionReasonFallback: 'Wallet Transaction',
  txCredit: 'Credit',
  txDebit: 'Debit',
  txFee: 'Fee',
  txRefund: 'Refund',
...
/**
 * Resolved strings for UI + voice. Unknown `lang` → 'en'. Missing keys → English (`en` + EXPO_UI_EN).
 * @param {string} lang
 * @returns {Record<string, string>}
 */
export function getStrings(lang) {
  const code = LANGUAGE_ORDER.includes(lang) ? lang : DEFAULT_LANG;
  const baseEn = { ...EXPERT_VOICE_DICT.en, ...EXPO_UI_EN };
  const loc = EXPERT_VOICE_DICT[code] || {};
  return { ...baseEn, ...loc };
}
