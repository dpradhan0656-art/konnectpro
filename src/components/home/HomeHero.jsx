...
export default function HomeHero({
  ...
  return (
    <section
      className="relative pt-6 pb-28 w-full max-w-[100vw] overflow-x-hidden overflow-y-visible min-h-0 bg-gradient-to-b from-emerald-950 via-emerald-900 to-emerald-950 rounded-b-3xl shadow-[0_24px_80px_-24px_rgba(0,0,0,0.65)] border-b border-white/5 box-border px-0"
    ...
    >
      <div className="absolute inset-0 z-0 opacity-100 pointer-events-none overflow-hidden rounded-b-3xl" aria-hidden="true">
        <div className="absolute top-0 right-0 w-[min(100vw,28rem)] h-[min(100vw,28rem)] bg-emerald-500/25 rounded-full blur-[100px] -mr-24 -mt-24" />
        <div className="absolute bottom-0 left-0 w-[min(90vw,24rem)] h-[min(90vw,24rem)] bg-amber-500/15 rounded-full blur-[90px] -ml-16 -mb-16" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[min(120vw,40rem)] h-[min(120vw,40rem)] bg-emerald-400/10 rounded-full blur-[120px]" />
      </div>
      ...
      <div className="relative z-10 px-4 sm:px-6 max-w-4xl mx-auto min-w-0 w-full box-border">
        <div className="flex items-center gap-3 mb-6 sm:mb-8 bg-white/5 backdrop-blur-xl border border-white/10 w-fit max-w-full px-4 py-2.5 rounded-full shadow-[0_8px_32px_rgba(0,0,0,0.25)] ring-1 ring-white/5">
          <div
            className={`p-1.5 rounded-full ${cityStatus.active ? 'bg-gradient-to-br from-emerald-400 to-emerald-600' : 'bg-slate-800'} shadow-lg border ${cityStatus.active ? 'border-emerald-400/30' : 'border-white/10'}`}
          >
            <MapPin
              size={14}
          ...
          <div>
            <p className="text-[9px] font-bold text-amber-400 uppercase tracking-widest leading-none mb-1">{cityStatus.message}</p>
            {isEditingLoc ? (
              <input
                autoFocus
              ...
              <button
                ...
                className="text-left text-sm font-bold text-white leading-none tracking-wide border-b border-transparent hover:border-amber-400/50 transition-all duration-300"
              >
                {locationName}
              </button>
            )}
      ...
      <div className="relative z-10 w-full mb-8 sm:mb-10">
        <div className="w-full min-w-0 bg-white text-center py-10 sm:py-12 md:py-14 px-5 sm:px-8 md:px-12 lg:px-16 border-y border-slate-200/90 shadow-[inset_0_1px_0_rgba(255,255,255,0.9)]">
          <h1 className="font-display font-extrabold text-gray-900 leading-[1.08] tracking-[-0.02em] text-[clamp(1.75rem,5.5vw,3.75rem)] max-w-4xl mx-auto">
            ...
            <span className="block text-emerald-700 mt-2 sm:mt-3">Right at Your Doorstep.</span>
          </h1>
          ...
          <button
            ...
            className="mt-7 sm:mt-9 inline-flex items-center justify-center bg-emerald-700 text-white font-semibold text-base rounded-xl px-10 py-4 shadow-lg shadow-emerald-700/30 hover:bg-emerald-800 hover:shadow-xl hover:shadow-emerald-700/35 transition-all duration-200 active:scale-[0.98] focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 focus-visible:ring-offset-2"
          >
            Book Your Expert
          </button>
        </div>
      ...
      <div className="relative z-10 px-4 sm:px-6 max-w-4xl mx-auto min-w-0 w-full box-border">
        <div className="relative z-30 transform translate-y-6 w-full max-w-full">
          <div className="relative rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 shadow-[0_16px_48px_rgba(0,0,0,0.35)] flex items-center p-2 min-h-[52px] w-full box-border ring-1 ring-white/5">
            <Search className="absolute left-4 sm:left-5 text-emerald-400/80" size={22} aria-hidden="true" />
            <input
              type="search"
              value={searchQuery}
            ...
            <button
              ...
              className="absolute right-2 sm:right-3 p-2.5 min-h-[44px] min-w-[44px] rounded-xl text-emerald-300 hover:bg-emerald-500/15 hover:text-emerald-200 transition-all duration-300 flex items-center justify-center border border-transparent hover:border-emerald-500/20"
